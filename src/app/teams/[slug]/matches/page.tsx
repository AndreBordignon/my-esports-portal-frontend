              ...
              <li key={`${serie + index}`} className={'relative p-4 my-0 md:my-2 bg-transparent md:bg-custom-background bg-cover justify-center border-t-2 md:border-none border-blue-700'}>
                ...
                <Link href={`/matches/${serie.slug}/details`}>
                  ...
                  <div className="flex relative z-10 py-2 justify-center container mx-auto">
                    <div className="flex">
                      <p className={`text-bold text-3xl ${serie.winner?.slug === serie.opponents[0]?.opponent?.slug ? 'text-green-500' : 'text-red-500'} my-auto`}>
                        {serie.results[0].score}
                      </p>
                      <div className="flex flex-col">
                        <Link
                          href={{
                            pathname: `/teams/${serie?.opponents[0]?.opponent.slug}/details`,
                            query: { titulo: `Time ${serie?.opponents[0]?.opponent.name}` },
                          }}
                        >
                      ...
                      <div>
                        <Link
                          href={{
                            pathname: `/teams/${serie?.opponents[1]?.opponent.slug}/details`,
                            query: { titulo: `Time ${serie?.opponents[1]?.opponent.name}` },
                          }}
                        >
                      ...
                      <p className={`text-bold text-3xl ${serie.winner?.slug === serie.opponents[1]?.opponent?.slug ? 'text-green-500' : 'text-red-500'} my-auto`}>
                        {serie.results[1].score}
                      </p>

                      <Link
                        className="py-2 hidden md:block absolute right-5 px-4 border-2 rounded-md bg-secondary hover:bg-button border-button text-white my-8"
                        href={`/matches/${serie.slug}/details`}
                      >
                        detalhes -&gt;
                      </Link>
                    </div>
