...
const TeamDetails = ({ params }: { params: { slug: string } }) => {
  ...
  return (
    <>
      <div className="absolute w-screen h-36 left-0 opacity-90 bg-black"></div>
      <div className="container relative mx-auto mt-8 block">
        <div className="pr-6 flex justify-between">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              {' '}
              {/* Adicionado margem esquerda para espaço */}
              <h1 className="text-3xl text-white font-semibold">
                {team.name} {team.acronym && <strong className="text-lg absolute ml-2">{`(${team?.acronym})`}</strong>}
              </h1>
              <ReactCountryFlag
                ...
                title={team.location}
              />
              <p className="m-0 p-0 text-xs text-white ml-0.5 font-bold">{team.location}</p>
            </div>
          </div>
          <div className="flex items-center">
            <Link
              href={{ pathname: 'matches', query: { titulo: `Partidas ${team.name}` } }}
              className="p-2 w-18 md:w-28 md:h-auto md:text-center text-xs md:text-md md:px-4 md:py-4 border-2 rounded-md bg-secondary hover:bg-button border-button text-white"
            >
              Matches
            </Link>
          </div>
            ...
            (player: any, index: number) =>
              index <= 4 && (
                <div key={index} className="mx-auto my-2 p-4 w-72 border-2 rounded-lg border-border">
                  {player.image_url && <Image className="rounded-full h-24 w-24 mx-auto object-scale-up mb-4" src={player.image_url} width="100" height="100" alt={player.name} />}
